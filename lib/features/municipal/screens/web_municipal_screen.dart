...
class _WebMunicipalContentState extends State<WebMunicipalContent> {
  ...
  List<_Service> get _services => [
    ...
    _Service(
      ...
      blurb: _t('Clinics, pharmacies, dentists', 'מרפאות, בתי מרקחת ורופאי שיניים'),
  ...
  List<_Service> get _visibleServices {
    ...
    return _services
        .where((s) => s.label.toLowerCase().contains(q) || s.blurb.toLowerCase().contains(q))
        .toList();
  }

  // ── Emergency and service numbers ──
  List<_Contact> get _contacts => [
    _Contact(name: _t('Police', 'משטרה'), number: '100', icon: IconsaxPlusLinear.shield_tick),
    _Contact(name: _t('Ambulance (MDA)', 'מגן דוד אדום'), number: '101', icon: IconsaxPlusLinear.health),
    _Contact(name: _t('Fire & Rescue', 'כבאות והצלה'), number: '102', icon: IconsaxPlusLinear.danger),
    _Contact(name: _t('Home Front Command', 'פיקוד העורף'), number: '104', icon: IconsaxPlusLinear.shield_security),
    _Contact(name: _t('Municipal Hotline', 'מוקד עירוני'), number: '106', icon: IconsaxPlusLinear.call),
    _Contact(name: _t('Electric Company', 'חברת החשמל'), number: '103', icon: IconsaxPlusLinear.flash_1),
  ];
                    ...
                    children: services.map((s) {
                      return SizedBox(
                        ...
                        child: _ServiceCard(
                          ...
                          onTap: s.route == null ? null : () => context.push(s.route!),
                        ),
                      );
                    }).toList(),
                  );
              ...
              builder: (context, constraints) {
                ...
                return Wrap(
                  ...
                  children: _contacts.map((c) {
                    return SizedBox(width: cardWidth, child: _ContactCard(contact: c));
                  }).toList(),
                );
              },
            ),
              ...
              Expanded(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _hoursRow(_t('Wednesday', 'יום ד׳'), _t('Closed to the public', 'סגור לקהל')),
...
class _Service {
  ...
  final IconData icon;
  final String? route;
  const _Service({required this.label, required this.blurb, required this.icon, this.route});
}

class _Contact {
  final String name, number;
  final IconData icon;
  const _Contact({required this.name, required this.number, required this.icon});
}
...
class _ServiceCard extends StatefulWidget {
  ...
  const _ServiceCard({required this.service, required this.comingSoonLabel, this.onTap});
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: Colors.white,
            border: Border.all(color: _hovered && live ? AppColors.midBlue : _kBorder),
          ...
          child: Row(
            ...
            children: [
              Container(
                ...
                child: Center(child: Icon(s.icon, size: 26, color: AppColors.midBlue)),
              ),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
              ...
              if (live)
                Padding(
                  ...
                  child: Icon(Icons.chevron_right,
                      size: 20, color: _hovered ? AppColors.midBlue : _kIconGrey),
                ),
            ],
          ),
        ),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Center(child: Icon(contact.icon, size: 22, color: AppColors.midBlue)),
          ),
          const SizedBox(width: 16),
          Expanded(
