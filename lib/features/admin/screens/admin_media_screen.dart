        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 12),
              _FilterChip('הכל', _mimeFilter.isEmpty, () {
                setState(() => _mimeFilter = '');
                ref.read(adminMediaListProvider.notifier).setMimeFilter(null);
              }),
              _FilterChip('JPEG', _mimeFilter == 'image/jpeg', () {
                setState(() => _mimeFilter = 'image/jpeg');
                ref
                    .read(adminMediaListProvider.notifier)
                    .setMimeFilter('image/jpeg');
              }),
              _FilterChip('PNG', _mimeFilter == 'image/png', () {
                setState(() => _mimeFilter = 'image/png');
                ref
                    .read(adminMediaListProvider.notifier)
                    .setMimeFilter('image/png');
              }),
              _FilterChip('WebP', _mimeFilter == 'image/webp', () {
                setState(() => _mimeFilter = 'image/webp');
                ref
                    .read(adminMediaListProvider.notifier)
                    .setMimeFilter('image/webp');
              }),
              const Spacer(),
              IconButton(
                icon: Icon(
                  _gridView ? Icons.grid_view : Icons.view_list,
                  size: 20,
          ...
          child: asyncData.when(
            ...
            data: (list) {
              ...
              if (_gridView) {
                return GridView.builder(
                  ...
                  itemBuilder: (_, i) => _MediaCard(
                    media: list[i],
                    onTap: () => _showEditor(context, list[i]),
              ...
              return _MediaTable(
                list: list,
                isWide: isWide,
                onEdit: (m) => _showEditor(context, m),
  ...
  void _showEditor(BuildContext context, Map<String, dynamic>? existing) {
    ...
    showDialog(
      ...
      builder: (ctx) => Directionality(
        ...
        child: AlertDialog(
          title: Text(
            existing == null ? 'העלאת קובץ' : 'עריכת מדיה',
            style: TextStyle(
              fontFamily: AppFonts.rubik,
                ...
                if (existing != null) ...[
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 12),
                      Text(
                        _formatSize(existing['file_size'] as int? ?? 0),
                        style: TextStyle(
                          fontFamily: AppFonts.rubik,
                          fontSize: 12,
                          color: AppColors.grayText,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                final data = {
                  'filename': filenameC.text,
                  'alt_text': altC.text,
                  'mime_type': 'image/jpeg',
                  'file_size': 100000,
                  'width': 800,
                  'height': 600,
                  'uploaded_by': 'ניתאי לוי',
                };
                if (existing != null) {
                  ref.read(adminMediaListProvider.notifier).updateMedia(
                    existing['id'] as String,
                    {'filename': filenameC.text, 'alt_text': altC.text},
                  );
                } else {
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ref
                    .read(adminMediaListProvider.notifier)
                    .deleteMedia(media['id'] as String);
                Navigator.pop(ctx);
              },
              style: ElevatedButton.styleFrom(
