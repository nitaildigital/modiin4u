...
class _CampaignEditorDialogState extends ConsumerState<_CampaignEditorDialog> {
  ...
  void initState() {
    ...
    _businessName = TextEditingController(
      text: c?['business_name'] as String? ?? '',
    );
    _businessId = TextEditingController(
      text: c?['business_id'] as String? ?? '',
    );
    _placementLabel = TextEditingController(
      text: c?['placement_label'] as String? ?? '',
    );
    _placementId = TextEditingController(
      text: c?['placement_id'] as String? ?? '',
    );
    ...
    _priority = TextEditingController(
      text: (c?['priority'] as int?)?.toString() ?? '5',
    );
    _frequencyCap = TextEditingController(
      text: (c?['frequency_cap'] as int?)?.toString() ?? '',
    );
    _salesperson = TextEditingController(
      text: c?['salesperson'] as String? ?? '',
    );
  ...
  void dispose() {
    ...
    _businessId.dispose();
    _placementLabel.dispose();
    _placementId.dispose();
    _destinationUrl.dispose();
    _startAt.dispose();
    _endAt.dispose();
    _priority.dispose();
    _frequencyCap.dispose();
